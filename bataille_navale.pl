...
% mise en mÈmoire des coups tirÈs
...
% on rÈcupËre tous les points du bateau gr‚ce ‡ son id et on vÈrifie si ils sont tous dans la liste des coups_tires
...
% mise en mÈmoire des coups tirÈs et des bateaux touchÈs 
...
% inutile de tirer dans une case esseulÈe
...
% on stocke tous les prÈdicats "bateaux_touches_joueur2" dans une liste
tirer_J2 :- findall([X, Y], bateaux_touches_joueur2(_, X, Y), R), tirer_J2(R).

% si la liste est vide on tire au hasard dans une case qui possËde au moins une case adjacente o˘ on n'a pas tirÈ 
...
% autrement on applique une stratÈgie
...
% lorsqu'il n'y a qu'un seul ÈlÈment touchÈ, on prend les points adjacents
% on supprime les points qui se situent hors du terrain et les coups dÈj‡ tirÈs
...
% trouve les extrÈmitÈs verticales ‡ partir des coordonnÈes en X des bateaux touchÈs
...
% trouve les extrÈmitÈs horizontales ‡ partir des coordonnÈes en Y des bateaux touchÈs
...
% alignement vertical : on essaye de tirer aux des extrÈmitÈs verticales
evaluer_alignement([X, Y], [T1|Q1], []) :- extremites_verticales([X, Y], [T1|Q1], XTrouves),
evaluer_tir([X, Y], XTrouves, []).

% alignement horizontal : on essaye de tirer aux extrÈmitÈs horizontales
...
%%%%%%%% Affichage map tir et touchÈ J1
...
%%%%%%%% Affichage map tir et touchÈ J2
...
% fonction prÈsente dans la version 6 de SWI-Prolog, mais pas dans la version 5
